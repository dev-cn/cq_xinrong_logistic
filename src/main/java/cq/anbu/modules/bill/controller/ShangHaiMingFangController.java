 ...
 */
@RestController
@RequestMapping("/bill/shanghaimingfang")
public class ShangHaiMingFangController extends AbstractController{
	@Autowired
	private ShangHaiMingFangService shangHaiMingFangService;
	 ...
	 */
	@RequestMapping("/list")
	@RequiresPermissions("bill:shanghaimingfang:list")
	public R list(@RequestParam Map<String, Object> params){
		//查询列表数据
        Query query = new Query(params);

		List<ShangHaiMingFangEntity> shangHaiMingFangList = shangHaiMingFangService.queryList(query);
		int total = shangHaiMingFangService.queryTotal(query);
		
		PageUtils pageUtil = new PageUtils(shangHaiMingFangList, total, query.getLimit(), query.getPage());
	...
	@RequiresPermissions("bill:shanghaimingfang:info")
	public R info(@PathVariable("id") Long id){
		ShangHaiMingFangEntity shangHaiMingFang = shangHaiMingFangService.queryObject(id);
		
		return R.ok().put("shangHaiMingFang", shangHaiMingFang);
	}
	 ...
	 */
	@RequestMapping("/save")
	@RequiresPermissions("bill:shanghaimingfang:save")
	public R save(@RequestBody ShangHaiMingFangEntity shangHaiMingFang){
		shangHaiMingFang = wrapperBaseEntity(shangHaiMingFang);
	...
	@RequiresPermissions("bill:shanghaimingfang:update")
	public R update(@RequestBody ShangHaiMingFangEntity shangHaiMingFang){
        shangHaiMingFang.setUpdateAt(new Date());
        shangHaiMingFang.setUpdateBy(getUser().getUsername());
		shangHaiMingFangService.update(shangHaiMingFang);
	...
	@RequiresPermissions("bill:shanghaimingfang:delete")
	public R delete(@RequestBody Long[] ids){
		shangHaiMingFangService.deleteBatch(ids);
...
//        目前只需单sheet导出
    ...
    private List<ShangHaiMingFangEntity> getShangHaiMingFangEntityList(String ids) {
        String[] idsArray = ids.split(",", -1);
        List<ShangHaiMingFangEntity> list = Lists.newArrayList();
        for (String id : idsArray) {
            ShangHaiMingFangEntity shangHaiMingFangEntity = shangHaiMingFangService.queryObject(Long.parseLong(id));
            list.add(shangHaiMingFangEntity);
        }
        return list;
    }
