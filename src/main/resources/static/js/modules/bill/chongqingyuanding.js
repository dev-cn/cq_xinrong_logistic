$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'bill/chongqingyuanding/list',
        datatype: "json",
        colModel: [			
    ...
    new AjaxUpload('#excelImport', {
        action: baseURL + 'bill/chongqingyuanding/excelImport?token=' + token,
        name: 'file',
        autoSubmit: true,
        responseType: "json",
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.chongQingYuanDing.id == null ? "bill/chongqingyuanding/save" : "bill/chongqingyuanding/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.chongQingYuanDing),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "bill/chongqingyuanding/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "bill/chongqingyuanding/info/"+id, function(r){
                vm.chongQingYuanDing = r.chongQingYuanDing;
            });
		},
		reload: function (event) {
			...
			$("#jqGrid").jqGrid('setGridParam',{
                postData: {'trackingNo': vm.chongQingYuanDing.trackingNo,'beginDate':beginDate,'endDate':endDate},
                page:page
            }).trigger("reloadGrid");
		},
        excelExport: function (event) {
            excelExport(baseURL + "bill/chongqingyuanding/export?token=" + token);
        },
        reset: function (event) {
			vm.chongQingYuanDing.trackingNo = null;
            $("#beginDate").val("");
            $("#endDate").val("");
            $("#trackingNo").val("");
            vm.reload();
...
function Datetime() {
    $('#datetimepicker1,#datetimepicker2').datetimepicker({
		...
		vm.chongQingYuanDing.deliveryDate = deliveryDate;
		vm.chongQingYuanDing.arrivalDate = arrivalDate;
    });
}