$(function () {
    ...
    new AjaxUpload('#excelImport', {
        action: baseURL + 'bill/xinsilu/excelImport?token=' + token,
        name: 'file',
        autoSubmit: true,
        responseType: "json",
...
var vm = new Vue({
    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            var url = vm.xinSiLu.id == null ? "bill/xinsilu/save" : "bill/xinsilu/update";
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
                data: JSON.stringify(vm.xinSiLu),
                success: function (r) {
                    if (r.code === 0) {
        ...
        del: function (event) {
            ...
            confirm('确定要删除选中的记录？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "bill/xinsilu/delete",
                    contentType: "application/json",
        ...
        getInfo: function (id) {
            $.get(baseURL + "bill/xinsilu/info/" + id, function (r) {
                vm.xinSiLu = r.xinSiLu;
            });
        },
        reload: function (event) {
            ...
            $("#jqGrid").jqGrid('setGridParam', {
                postData: {'trackingNo': vm.xinSiLu.trackingNo, 'beginDate': beginDate, 'endDate': endDate},
                page: page
            }).trigger("reloadGrid");
        },
        excelExport: function (event) {
            excelExport(baseURL + "bill/xinsilu/export?token=" + token);
        },
        reset: function (event) {
            vm.xinSiLu.trackingNo = null;
            $("#beginDate").val("");
            $("#endDate").val("");
            $("#trackingNo").val("");
            vm.reload();
...
function Datetime() {
    $('#datetimepicker1,#datetimepicker2').datetimepicker({
        ...
        vm.xinSiLu.deliveryDate = deliveryDate;
        vm.xinSiLu.arrivalDate = arrivalDate;
    });
}