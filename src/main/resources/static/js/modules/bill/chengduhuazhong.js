$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'bill/chengduhuazhong/list',
        datatype: "json",
        colModel: [			
			...
			{ label: '提货地点', name: 'deliveryAddr', index: 'delivery_addr', width: 120 },
    ...
    new AjaxUpload('#excelImport', {
        action: baseURL + 'bill/chengduhuazhong/excelImport?token=' + token,
        name: 'file',
        autoSubmit: true,
        responseType: "json",
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.chengDuHuaZhong.id == null ? "bill/chengduhuazhong/save" : "bill/chengduhuazhong/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.chengDuHuaZhong),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "bill/chengduhuazhong/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "bill/chengduhuazhong/info/"+id, function(r){
                vm.chengDuHuaZhong = r.chengDuHuaZhong;
            });
		},
		reload: function (event) {
			...
			$("#jqGrid").jqGrid('setGridParam',{
                postData: {'trackingNo': vm.chengDuHuaZhong.trackingNo,'beginDate':beginDate,'endDate':endDate},
                page:page
            }).trigger("reloadGrid");
		},
        excelExport: function (event) {
            excelExport(baseURL + "bill/chengduhuazhong/export?token=" + token);
        },
        reset: function (event) {
			vm.chengDuHuaZhong.trackingNo = null;
            $("#beginDate").val("");
            $("#endDate").val("");
            $("#trackingNo").val("");
            vm.reload();
...
function Datetime() {
    $('#datetimepicker1,#datetimepicker2').datetimepicker({
		...
		vm.chengDuHuaZhong.deliveryDate = deliveryDate;
		vm.chengDuHuaZhong.arrivalDate = arrivalDate;
    });
}