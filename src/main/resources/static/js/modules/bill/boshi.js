$(function () {
    ...
    new AjaxUpload('#excelImport', {
        action: baseURL + 'bill/boshi/excelImport?token=' + token,
        name: 'file',
        autoSubmit: true,
        responseType: "json",
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.boShi.id == null ? "bill/boshi/save" : "bill/boshi/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.boShi),
			    success: function(r){
			    	if(r.code === 0){
		...
		getInfo: function(id){
			$.get(baseURL + "bill/boshi/info/"+id, function(r){
                vm.boShi = r.boShi;
            });
		},
		reload: function (event) {
			...
			$("#jqGrid").jqGrid('setGridParam',{
                postData: {'trackingNo': vm.boShi.trackingNo,'beginDate':beginDate,'endDate':endDate},
                page:page
            }).trigger("reloadGrid");
		},
        excelExport: function (event) {
            excelExport(baseURL + "bill/boshi/export?token=" + token);
        },
        reset: function (event) {
			vm.boShi.trackingNo = null;
            $("#beginDate").val("");
            $("#endDate").val("");
            $("#trackingNo").val("");
            vm.reload();
...
function Datetime() {
    $('#datetimepicker1,#datetimepicker2').datetimepicker({
		...
		vm.boShi.deliveryDate = deliveryDate;
		vm.boShi.arrivalDate = arrivalDate;
    });
}