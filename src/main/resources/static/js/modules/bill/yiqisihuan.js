$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'bill/yiqisihuan/list',
        datatype: "json",
        colModel: [			
    ...
    new AjaxUpload('#excelImport', {
        action: baseURL + 'bill/yiqisihuan/excelImport?token=' + token,
        name: 'file',
        autoSubmit: true,
        responseType: "json",
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.yiQiSiHuan.id == null ? "bill/yiqisihuan/save" : "bill/yiqisihuan/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.yiQiSiHuan),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "bill/yiqisihuan/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "bill/yiqisihuan/info/"+id, function(r){
                vm.yiQiSiHuan = r.yiQiSiHuan;
            });
		},
		reload: function (event) {
			...
			$("#jqGrid").jqGrid('setGridParam',{
                postData: {'trackingNo': vm.yiQiSiHuan.trackingNo,'beginDate':beginDate,'endDate':endDate},
                page:page
            }).trigger("reloadGrid");
		},
        excelExport: function (event) {
            excelExport(baseURL + "bill/yiqisihuan/export?token=" + token);
        },
        reset: function (event) {
			vm.yiQiSiHuan.trackingNo = null;
            $("#beginDate").val("");
            $("#endDate").val("");
            $("#trackingNo").val("");
            vm.reload();
...
function Datetime() {
    $('#datetimepicker1,#datetimepicker2').datetimepicker({
		...
		vm.yiQiSiHuan.deliveryDate = deliveryDate;
		vm.yiQiSiHuan.arrivalDate = arrivalDate;
    });
}