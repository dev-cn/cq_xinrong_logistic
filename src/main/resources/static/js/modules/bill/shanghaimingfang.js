$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'bill/shanghaimingfang/list',
        datatype: "json",
        colModel: [
    ...
    new AjaxUpload('#excelImport', {
        action: baseURL + 'bill/shanghaimingfang/excelImport?token=' + token,
        name: 'file',
        autoSubmit: true,
        responseType: "json",
...
var vm = new Vue({
    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            var url = vm.shangHaiMingFang.id == null ? "bill/shanghaimingfang/save" : "bill/shanghaimingfang/update";
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
                data: JSON.stringify(vm.shangHaiMingFang),
                success: function (r) {
                    if (r.code === 0) {
        ...
        del: function (event) {
            ...
            confirm('确定要删除选中的记录？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "bill/shanghaimingfang/delete",
                    contentType: "application/json",
        ...
        getInfo: function (id) {
            $.get(baseURL + "bill/shanghaimingfang/info/" + id, function (r) {
                vm.shangHaiMingFang = r.shangHaiMingFang;
            });
        },
        reload: function (event) {
            ...
            $("#jqGrid").jqGrid('setGridParam', {
                postData: {'trackingNo': vm.shangHaiMingFang.trackingNo, 'beginDate': beginDate, 'endDate': endDate},
                page: page
            }).trigger("reloadGrid");
        },
        excelExport: function (event) {
            excelExport(baseURL + "bill/shanghaimingfang/export?token=" + token);
        },
        reset: function (event) {
            vm.shangHaiMingFang.trackingNo = null;
            $("#beginDate").val("");
            $("#endDate").val("");
            $("#trackingNo").val("");
            vm.reload();
...
function Datetime() {
    $('#datetimepicker1,#datetimepicker2').datetimepicker({
        ...
        vm.shangHaiMingFang.deliveryDate = deliveryDate;
        vm.shangHaiMingFang.arrivalDate = arrivalDate;
    });
}