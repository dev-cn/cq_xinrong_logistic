...
var vm = new Vue({
    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            var url = vm.boFanKeJi.id == null ? "bill/bofankeji/save" : "bill/bofankeji/update";
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
                data: JSON.stringify(vm.boFanKeJi),
                success: function (r) {
                    if (r.code === 0) {
        ...
        del: function (event) {
            ...
            confirm('确定要删除选中的记录？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "bill/bofankeji/delete",
                    contentType: "application/json",
        ...
        getInfo: function (id) {
            $.get(baseURL + "bill/bofankeji/info/" + id, function (r) {
                vm.boFanKeJi = r.boFanKeJi;
            });
        },
        reload: function (event) {
            ...
            $("#jqGrid").jqGrid('setGridParam', {
                postData: {'trackingNo': vm.boFanKeJi.trackingNo, 'beginDate': beginDate, 'endDate': endDate},
                page: page
            }).trigger("reloadGrid");
        },
        excelExport: function (event) {
            excelExport(baseURL + "bill/bofankeji/export?token=" + token);
        },
        reset: function (event) {
            vm.boFanKeJi.trackingNo = null;
            $("#beginDate").val("");
            $("#endDate").val("");
            $("#trackingNo").val("");
            vm.reload();
