$(function () {
    ...
    new AjaxUpload('#excelImport', {
        action: baseURL + 'bill/saweiao/excelImport?token=' + token,
        name: 'file',
        autoSubmit: true,
        responseType: "json",
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.saWeiAo.id == null ? "bill/saweiao/save" : "bill/saweiao/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.saWeiAo),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "bill/saweiao/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "bill/saweiao/info/"+id, function(r){
                vm.saWeiAo = r.saWeiAo;
            });
		},
		reload: function (event) {
			...
			$("#jqGrid").jqGrid('setGridParam',{
                postData: {'trackingNo': vm.saWeiAo.trackingNo,'beginDate':beginDate,'endDate':endDate},
                page:page
            }).trigger("reloadGrid");
		},
        excelExport: function (event) {
            excelExport(baseURL + "bill/saweiao/export?token=" + token);
        },
        reset: function (event) {
			vm.saWeiAo.trackingNo = null;
            $("#beginDate").val("");
            $("#endDate").val("");
            $("#trackingNo").val("");
            vm.reload();
...
function Datetime() {
    $('#datetimepicker1,#datetimepicker2').datetimepicker({
		...
		vm.saWeiAo.deliveryDate = deliveryDate;
		vm.saWeiAo.arrivalDate = arrivalDate;
    });
}