$(function () {
    ...
    new AjaxUpload('#excelImport', {
        action: baseURL + 'bill/siweier/excelImport?token=' + token,
        name: 'file',
        autoSubmit: true,
        responseType: "json",
...
var vm = new Vue({
    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            var url = vm.siWeiEr.id == null ? "bill/siweier/save" : "bill/siweier/update";
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
                data: JSON.stringify(vm.siWeiEr),
                success: function (r) {
                    if (r.code === 0) {
        ...
        del: function (event) {
            ...
            confirm('确定要删除选中的记录？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "bill/siweier/delete",
                    contentType: "application/json",
        ...
        getInfo: function (id) {
            $.get(baseURL + "bill/siweier/info/" + id, function (r) {
                vm.siWeiEr = r.siWeiEr;
            });
        },
        reload: function (event) {
            ...
            $("#jqGrid").jqGrid('setGridParam', {
                postData: {'trackingNo': vm.siWeiEr.trackingNo, 'beginDate': beginDate, 'endDate': endDate},
                page: page
            }).trigger("reloadGrid");
        },
        excelExport: function (event) {
            excelExport(baseURL + "bill/siweier/export?token=" + token);
        },
        reset: function (event) {
            vm.siWeiEr.trackingNo = null;
            $("#beginDate").val("");
            $("#endDate").val("");
            $("#trackingNo").val("");
            vm.reload();
...
function Datetime() {
    $('#datetimepicker1,#datetimepicker2').datetimepicker({
        ...
        vm.siWeiEr.deliveryDate = deliveryDate;
        vm.siWeiEr.arrivalDate = arrivalDate;
    });
}